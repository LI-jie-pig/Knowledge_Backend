...
milvus的安装需在docker上安装三个服务，milvus-standalone，milvus-etcd，milvus-minio
当你向 Milvus 插入一批向量数据时：
请求首先打到 standalone（管理员）。
standalone 把“这批数据属于哪个集合、字段是什么”这类元信息，记录进 etcd（卡片柜）。
然后，standalone 把真正的向量数据和索引文件，写进 minio（大书库）。
-------------------  rag开发 -------------------
__init__.py 文件的作用：
1.使 Python 解释器将目录识别为一个包。
2.定义包对外暴露的接口，即当其他模块导入该包时，只能导入包中 __init__.py 文件中定义的模块或函数。
3.包被导入时自动执行里面代码，可以用于初始化操作。

使用_开头的变量和方法，都表示私有变量和方法，只能在包内访问，包外不能访问。
global关键字，表明要修改模块级的 _client 变量，这里的_client类似java的静态变量

isinstance关键字类似于java的instanceof
yield关键字，在循环的时候，每次循环都会返回一个值，下次循环会从上次返回的地方继续执行，直到循环结束。
yield关键字可以用于生成器，生成器可以用于创建迭代器，迭代器可以用于遍历数据。
